...
def load_adapters(s_path_to_the_file_with_adapters):
    ...
    s_is_this_a_barcode = "no" #if I'm currently looking at a barcode, I'm changing this variable to "yes".
    s_adapter_title = "" #For ">SQK-MAP006|right" this variable will contain "SQK-MAP006"
    s_adapter_location = "" #"left", if this adapter (or barcode) is to be sought at the left end of a read. "right" if at the right end.
    
    d_adapter_title_to_its_left_sequence = {} #a dictionary with a key like "SQK-MAP006" and a value is the sequence which is to be sought at the left end of a read, like "GGTTGTTTCTGTTGGTGCTGATATTGCT".
    d_adapter_title_to_its_right_sequence = {} #a dictionary with a key like "SQK-MAP006" and a value is the sequence which is to be sought at the left end of a read, like "GCAATATCAGCACCAACAGAAA".
    
    #reading titles and sequences of all adapters (and barcodes)
    for s_line in f_infile:
        if re.search(r"^>(.+)\|(left|right)", s_line):
            o_regular_expression_results = re.search(r"^>(.+)\|(left|right)", s_line)
            s_adapter_title = o_regular_expression_results.group(1)
            s_adapter_location = o_regular_expression_results.group(2)            
        
        if re.search(r"^[A-Za-z\s]+$", s_line):
            s_adapter_sequence = s_line
            s_adapter_sequence = re.sub(r"[\r\n\s]", "", s_adapter_sequence) #removing line breaks and whitespace characters.
            if s_adapter_location == "left":
                d_adapter_title_to_its_left_sequence[s_adapter_title] = s_adapter_sequence
            if s_adapter_location == "right":
                d_adapter_title_to_its_right_sequence[s_adapter_title] = s_adapter_sequence
            
        if re.search(r"^>", s_line) and (not re.search(r"(left|right)", s_line)):
            sys.exit("Error: didn't find '|left' or '|right' in the title of the following adapter: " + s_adapter_title + " . See the instructions on how to name adapters at https://github.com/shelkmike/NTproc\n")
    
    #filling the list "ADAPTERS"
    for s_adapter_title in d_adapter_title_to_its_left_sequence:
        #if there is also a "right" version of this left adapter.
        if s_adapter_title in d_adapter_title_to_its_right_sequence:
        
            if re.search(r"^BC", s_adapter_title) and re.search(r"_rev$", s_adapter_title): #if this is a barcode and it is reverse. In the future, I'll probably have to document that reverse barcodes should have "_rev" in their titles. For some reasons it matters for Porechop whether barcodes are direct or reverse (I didn't examine this in detail).
                s_adapter_title_without_rev = re.sub(r"_rev$", "", s_adapter_title) #removing "rev" from the adapter title. Otherwise Porechop will work incorrectly.
                n_barcode_number = int(re.sub(r"^BC[0]*", "", s_adapter_title_without_rev)) #The number of a barcode. For "BC07" this will be "7".
                ADAPTERS.append(Adapter("Barcode " + str(n_barcode_number) + " (reverse)", start_sequence=(s_adapter_title_without_rev + "_rev", d_adapter_title_to_its_left_sequence[s_adapter_title]), end_sequence=(s_adapter_title_without_rev, d_adapter_title_to_its_right_sequence[s_adapter_title]))) 
            elif re.search(r"^BC", s_adapter_title) and (not re.search(r"_rev$", s_adapter_title)): #if this is a barcode and it is not reverse.
                n_barcode_number = int(re.sub(r"^BC[0]*", "", s_adapter_title)) #The number of a barcode. For "BC07" this will be "7".
                ADAPTERS.append(Adapter("Barcode " + str(n_barcode_number) + " (forward)", start_sequence=(s_adapter_title, d_adapter_title_to_its_left_sequence[s_adapter_title]), end_sequence=(s_adapter_title + "_rev", d_adapter_title_to_its_right_sequence[s_adapter_title])))
            else: #if this is not a barcode but an adapter.
                ADAPTERS.append(Adapter(s_adapter_title, start_sequence=(s_adapter_title + "|left", d_adapter_title_to_its_left_sequence[s_adapter_title]), end_sequence=(s_adapter_title + "|right", d_adapter_title_to_its_right_sequence[s_adapter_title])))
        else:
            if re.search(r"^BC", s_adapter_title) and re.search(r"_rev$", s_adapter_title): #if this is a barcode and it is reverse. In the future, I'll probably have to document that reverse barcodes should have "_rev" in their titles. For some reasons it matters for Porechop whether barcodes are direct or reverse (I didn't examine this in detail).
                s_adapter_title_without_rev = re.sub(r"_rev$", "", s_adapter_title) #removing "rev" from the adapter title. Otherwise Porechop will work incorrectly.
                n_barcode_number = int(re.sub(r"^BC[0]*", "", s_adapter_title_without_rev)) #The number of a barcode. For "BC07" this will be "7".
                ADAPTERS.append(Adapter("Barcode " + str(n_barcode_number) + " (reverse)", start_sequence=(s_adapter_title_without_rev + "_rev", d_adapter_title_to_its_left_sequence[s_adapter_title]))) 
            elif re.search(r"^BC", s_adapter_title) and (not re.search(r"_rev$", s_adapter_title)): #if this is a barcode and it is not reverse.
                n_barcode_number = int(re.sub(r"^BC[0]*", "", s_adapter_title)) #The number of a barcode. For "BC07" this will be "7".
                ADAPTERS.append(Adapter("Barcode " + str(n_barcode_number) + " (forward)", start_sequence=(s_adapter_title, d_adapter_title_to_its_left_sequence[s_adapter_title])))
            else: #if this is not a barcode but an adapter.
                ADAPTERS.append(Adapter(s_adapter_title, start_sequence=(s_adapter_title + "|left", d_adapter_title_to_its_left_sequence[s_adapter_title])))
    #now filling the list "ADAPTERS" for all adapters (or barcodes) that have the right sequence but not the left
    for s_adapter_title in d_adapter_title_to_its_right_sequence:
            #if there is also a "right" version of this left adapter.
            if s_adapter_title not in d_adapter_title_to_its_right_sequence:
                if re.search(r"^BC", s_adapter_title) and re.search(r"_rev$", s_adapter_title): #if this is a barcode and it is reverse. In the future, I'll probably have to document that reverse barcodes should have "_rev" in their titles. For some reasons it matters for Porechop whether barcodes are direct or reverse (I didn't examine this in detail).
                    s_adapter_title_without_rev = re.sub(r"_rev$", "", s_adapter_title) #removing "rev" from the adapter title. Otherwise Porechop will work incorrectly.
                    n_barcode_number = int(re.sub(r"^BC[0]*", "", s_adapter_title_without_rev)) #The number of a barcode. For "BC07" this will be "7".
                    ADAPTERS.append(Adapter("Barcode " + str(n_barcode_number) + " (reverse)", start_sequence=(s_adapter_title_without_rev + "_rev", d_adapter_title_to_its_right_sequence[s_adapter_title]))) 
                elif re.search(r"^BC", s_adapter_title) and (not re.search(r"_rev$", s_adapter_title)): #if this is a barcode and it is not reverse.
                    n_barcode_number = int(re.sub(r"^BC[0]*", "", s_adapter_title)) #The number of a barcode. For "BC07" this will be "7".
                    ADAPTERS.append(Adapter("Barcode " + str(n_barcode_number) + " (forward)", start_sequence=(s_adapter_title, d_adapter_title_to_its_right_sequence[s_adapter_title])))
                else: #if this is not a barcode but an adapter.
                    ADAPTERS.append(Adapter(s_adapter_title, start_sequence=(s_adapter_title + "|left", d_adapter_title_to_its_right_sequence[s_adapter_title])))
    f_infile.close()
    return ADAPTERS
...
def fix_up_1d2_sets(matching_sets):
    ...
    if '1D^2_part_1' in matching_set_names and '1D^2_part_2' in matching_set_names and \
            'SQK-MAP006_short' in matching_set_names:
        part_1_score = [x for x in matching_sets
                        if x.name == '1D^2_part_1'][0].best_start_or_end_score()
        part_2_score = [x for x in matching_sets
                        if x.name == '1D^2_part_2'][0].best_start_or_end_score()
        sqk_score = [x for x in matching_sets
                     if x.name == 'SQK-MAP006_short'][0].best_start_or_end_score()
        if part_1_score >= sqk_score and part_2_score >= sqk_score:
            matching_sets = [x for x in matching_sets if x.name != 'SQK-MAP006_short']
